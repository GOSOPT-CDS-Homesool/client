import { AllReviewDataType } from "../type/allReviewDataType";

export const AllReviewData: Array<AllReviewDataType> = [
  {
    id: 1,
    image: "https://homesool.s3.ap-northeast-2.amazonaws.com/review_1.png",
    title: "패키지 디자인 너무 예뻐요.. ",
    contents:
      "겉 뿐만 아니라 안에 같이 들어있는 내용물들도 디자인에 신경 쓴 모습이 보이네요. 만들면서 기분도 좋아졌습니다~ 역시 믿고 사는 느린마을!",
    star: 5,
  },
  {
    id: 2,
    image: "https://homesool.s3.ap-northeast-2.amazonaws.com/review_2.png",
    title: "사먹는 막걸리 맛은 안납니다",
    contents:
      "집안 습도, 온도 같은 변수들이 많아서 그런지 기대했던 느린마을 막걸리 맛은 안나네요. 그래도 만드는 재미가 있어서 4점 남깁니다.",
    star: 4,
  },
  {
    id: 3,
    image: "https://homesool.s3.ap-northeast-2.amazonaws.com/review_3.png",
    title: "어버이날 선물로 제격입니다.",
    contents:
      "아버지가 막걸리를 좋아하셔서 나무잔까지 세트로 되어있는 느린마을 막걸리를 보고 바로 주문했는데, 너무 좋아하시네요! 5개에 나무잔까지 2만원 초중반에 구매할 수 있다는 것도 가성비 굿! 선물용으로 한 번 구매해보세요",
    star: 5,
  },
  {
    id: 4,
    image: "https://homesool.s3.ap-northeast-2.amazonaws.com/review_4.png",
    title: "굿 !!! 느린마을 입문했네요",
    contents:
      "다른 막걸리보다 느린마을 향이나 맛이 너무 깊다는 이야기를 들어 구매했는데, 너무 만족스럽습니다! 전이랑 잘 어울려서 파전, 김치전 ... 계속 남편이랑 해먹고 있네요 ㅎㅎ 유통기한이 살짝 아쉽긴 하지만 재구매 의사 있습니다! 나무잔도 넘 귀여워요 ",
    star: 4,
  },
  {
    id: 5,
    image: "https://homesool.s3.ap-northeast-2.amazonaws.com/review_5.png",
    title: "이 가격에 막걸리 키트를? 너무 좋아요ㅠㅠ",
    contents: "반짝홈술에 막걸리 킷 떠가지고 30% 할인하길래 바로 샀어요~ 얼른 숙성해서 혼술하고 싶네요~",
    star: 5,
  },
  {
    id: 6,
    image: "https://homesool.s3.ap-northeast-2.amazonaws.com/review_6.png",
    title: "배송일 변경이 아쉽습니다.",
    contents:
      "구독으로 정기 배송 오는건 너무 좋은데 배송일 변경이 최근 택배사 사정으로 안된다네요. 느린마을은 시간에 따라 맛도 변하는데 .. 시스템 개선이 필요해보여요 ",
    star: 3,
  },
];
